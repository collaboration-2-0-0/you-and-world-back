...
export const MemberInfoReqSchema = {
  ...
  member_desire: [Joi.string().min(0).max(1024), JOI_NULL],
  member_goal: [Joi.string().min(0).max(1024), JOI_NULL],
  member_activity: [Joi.string().min(0).max(1024), JOI_NULL],
  member_role: [Joi.string().min(0).max(255), JOI_NULL],
